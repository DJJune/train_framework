...
class Decoder(nn.Module):
    ...
    def __init__(self, embedding_size, num_block, output_size, heads=4, pretrain_embedding_model=None):
        """
        Args:
            embedding_size: dimension of embedding
            num_block:  number of blocks
        ...
        self.dotattn_layers = clones(
            Attention(embedding_size, h=heads), num_block)
        self.ffns = clones(FFN(embedding_size), num_block)
        self.linear = Linear(embedding_size, output_size)
...
class Transformer(nn.Module):
    ...
    def __init__(self, embedding_size, num_block, heads=4):
        super(Transformer, self).__init__()
        # the default number of attention heads is 4
        self.encoder = Encoder(embedding_size, num_block, heads)
        # self.decoder = Decoder(embedding_size, num_block, output_size)

    def forward(self, source_seq, source_pos):
        """
        Args:
        ...
        return memory, c_mask, attns_enc
